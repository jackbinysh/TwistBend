...
void RefineCurve(Link& Curve)
{
    ...
    // try this!!
    NewCurve.minx = Curve.minx;
    NewCurve.maxx = Curve.maxx;
    NewCurve.miny = Curve.miny;
    NewCurve.maxy = Curve.maxy;
...
{
     scale[0] = BoxFractionx/( (maxxin-minxin)/(Lx) );
     scale[1] = BoxFractiony/( (maxyin-minyin)/(Ly) );
     scale[2] = BoxFractionz/( (maxzin-minzin)/(Lz) );
  ...
  if(Scaling)
    {
	  ...
	  for(int s=0; s<CurveNP.Components[i].knotcurve.size(); s++)
            {
	      CurveNP.Components[i].knotcurve[s].xcoord = scale[0]*(CurveNP.Components[i].knotcurve[s].xcoord - midpoint[0]);
	      CurveNP.Components[i].knotcurve[s].ycoord = scale[1]*(CurveNP.Components[i].knotcurve[s].ycoord - midpoint[1]);
	      CurveNP.Components[i].knotcurve[s].zcoord = scale[2]*(CurveNP.Components[i].knotcurve[s].zcoord - midpoint[2]);
            }
        }
    }
}
